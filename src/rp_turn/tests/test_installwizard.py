...
def dummy_cmd_contents(
    cmd, saved_config, nics, encoding=None
...
class DummyStep:  # pylint: disable=too-few-public-methods
    ...
    def run(self, config, step_id, total_steps):
        ...
        config[f"testitem{self._my_test_id}"] = f"testvalue{self._my_test_id}"
        ...
        for invalid_config in invalid_configs:
            ...
            for required_key in required_keys:
                if required_key not in invalid_config.keys():
                    self.assertIn(f"  - {required_key} is a required key", output)
                else:
                    self.assertIn(
                        f"  - {required_key} has invalid value bad_value",
                        output,
                    )
        ...
        for multi_key in multi_keys:
            ...
            for invalid_config in invalid_configs:
                ...
                for unknown_key in invalid_config.keys():
                    self.assertIn(f"  - {unknown_key} is an unknown key", output)
