...
from si.apps.reverseproxy.steps import Step
from si.apps.reverseproxy.steps.routes import RoutesStep
...
class NetworkStep(Step):
    ...
    def __init__(self, nic_name, nic_mac, is_external, is_internal, nic_str=None):
        Step.__init__(self, "Configuring {} ({})".format(nic_name, nic_mac))
        assert is_external or is_internal
        self.nic_name = nic_name
        self.nic_mac = nic_mac
        self.nic_str = (
            nic_str if nic_str is not None else " for {} ({})".format(nic_name, nic_mac)
        )
    ...
    def _get_ip_address(self, config):
        ...
        response = self.ask("IP Address{}?".format(self.nic_str), default=default_ip)
    ...
    def _get_netmask(self, config):
        ...
        response = self.ask(
            "Subnet mask{}?".format(self.nic_str), default=default_netmask
        )
        DEV_LOGGER.info("Response: %s", response)
    ...
    def _get_gateway(self, config):
        ...
        response = self.ask(
            "Default gateway{}?".format(self.nic_str), default=default_gateway
        )
        DEV_LOGGER.info("Response: %s", response)
