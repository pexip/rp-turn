...
class RoutesStep(MultiStep):
    ...
    def __init__(self, nic_name, nic_mac):
        MultiStep.__init__(
            self,
            "Custom Routes for {} ({})".format(nic_name, nic_mac),
    ...
    def format_value(self, value):
        return "{} via {}".format(value["to"], value["via"])
    ...
    def _get_another_answer(self, config):
        ...
        response = self.ask_yes_no(
            "Add {} custom network route for {}?".format(another, self.nic_name)
        )
        if response:
            self.questions += [
    ...
    def _get_via_address(self, _):
        ...
        if not self._nic_interface.network.overlaps(IPv4Network(via_address)):
            raise StepError(
                "{} is not reachable from {} ({})".format(
                    via_address, self.nic_name, self._nic_interface.exploded
                )
            )
    ...
    def verify_route(value):
        ...
        try:
            ...
            raise StepError('Route needs a "to" network and a "via" address')
