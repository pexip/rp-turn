...
from si.apps.reverseproxy.steps.network import NetworkStep
...
class DualNicStep(Step):
    ...
    def _intro_dual_nic(self, _):
        ...
        for value in [
            nic_name + " ({})".format(nic_mac) for nic_name, nic_mac in self._nics
        ]:
            msg += "\n  - " + value
        self.display(msg)
    ...
    def _get_nic_name(self, config, internal=False, external=False):
        ...
        if internal and external:
            ...
            nic_type_str = "Which is the {}-facing network interface".format(
