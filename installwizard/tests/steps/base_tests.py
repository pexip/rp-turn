...
import si.apps.reverseproxy.steps.base_step
from si.apps.reverseproxy import utils
from si.apps.reverseproxy.tests import utils as test_utils
from si.apps.reverseproxy.step_error import StepError
from si.apps.reverseproxy.tests.utils import (
    VALID_YES_CASES,
    VALID_NO_CASES,
    INVALID_YESNO_CASES,
    VALID_DOMAIN_NAMES,
...
class QuestionUtils(twisted.trial.unittest.TestCase):
    ...
    def setUp(self):
        self._step = partial(
            si.apps.reverseproxy.steps.base_step.Step, "DummyQuestionUtils"
        )
        self._state_id = None
        self._question = None
        self._valid_cases = []
    ...
    def setup_question(self, case, step=None, question_str=None):
        ...
        if step is None:
            ...
            def fakeask(message, default=None):
                ...
                fake_out = StringIO()
                sys.stdout = fake_out
                output = step._ask(message, default=default)
                sys.stdout = sys.__stdout__
                return output

            step._ask = copy.copy(step.ask)
...
class TestQuestion(QuestionUtils):
    ...
    def setUp(self):
        self._step = partial(si.apps.reverseproxy.steps.base_step.Step, "DummyStep")
        self._state_id = None
        self._question = None
        self._valid_cases = []
        self._invalid_cases = []
...
class TestMultiQuestion(TestQuestion):
    ...
    def setUp(self):
        self._state_id = None
        self._step = partial(
            si.apps.reverseproxy.steps.base_step.MultiStep, "DummyStep", self._state_id
        )
    ...
    def test_use_default(self):
        ...
        utils.set_config_value_by_path(config, step._keyname, default_values)
        fake_out = StringIO()
        sys.stdout = fake_out
